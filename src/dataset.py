...
class SentimentDataset(Dataset):
    def __init__(self, dataframe, tokenizer, max_length=MAX_LENGTH):
        ...
        if "text" in self.dataframe.columns:
            ...
            raise ValueError("No text column found. Expected 'text' or 'field'.")

    def __len__(self):
        return len(self.dataframe)

    def __getitem__(self, index):
        text = str(self.dataframe.loc[index, self.text_column])
        ...
        # Add labels only if available
        if "sentiment" in self.dataframe.columns:
            label = (
                str(self.dataframe.loc[index, "sentiment"])
                .strip()
                .lower()
            )
...
def load_data():
    ...
    return train_df, valid_df, test_df, unlabelled_df
...
def get_datasets():
    ...
    unlabelled_dataset = SentimentDataset(unlabelled_df, tokenizer)
...
if __name__ == "__main__":
    ...
    print(f"Unlabelled samples : {len(unlabelled_dataset)}")
